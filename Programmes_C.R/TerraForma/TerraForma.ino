...
*/
  ...
  FileCreate(now, datafile, recentfile);

  PrintHeaders(datafile); //Print the header line(s) to the file.
...
void loop(){
  ...
  if (reading_request_phase) {   // Ask for reading
    ...
    if (millis() >= next_poll_time) {  //and its time to get the response
      ...
      //Save data to file.
      PrintToFile(rtc, datafile, recentfile, AbsPressure, Decibars, Meters, Celsius, sal_float,
                 nbSamples, vbatt, EC, PH, ORP, DO, average_color_readings, colorList);
