...
float get_pressure_depth(MS5837 psensor) {
  ...
  float AbsPressure = psensor.pressure();
...
float get_depth_meters(float AbsPressure, float AtmP, float latitude) {
  ...
  float Meters = ((((-1.82e-15 * decibars + 2.279e-10) * decibars - 2.2512e-5) * decibars + 9.72659) * decibars) / gr;  //Depth in meters.
  return Meters;
}
 ...
 * @return
 */
void PrintHeaders(File datafile, int USE_ATLAS) {  //Prints a header line to the CSV for column identification.
  if (datafile) {
    ...
    if (data == 1) {  // Corrected '=' to '=='
        ...
        if (datafile) {
            ...
            for (int j = 0; j < 10; j++) {
                ...
                datafile.print(average_color_readings[j], 7);  // Assuming colorList[j] is meant to be just j
                datafile.print(",");
            }
