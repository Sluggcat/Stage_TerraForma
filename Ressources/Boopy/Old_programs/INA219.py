...
class INA:
    ...
    def energy( self, data_list, index_u, index_i ):
        buf = bytearray(2)
        self.i2c.mem_read(buf, self.addr, INA.BUS_V)
        res = INA.bytes_to_int(buf[0], buf[1])
        volt = (res>>3) * 4 / 1000
        volt_str = str( "%.3f" % volt )
        data_list[index_u] = volt_str.replace('.', ',') 
        self.i2c.mem_read(buf, self.addr, INA.CURRENT)
        current = INA.bytes_to_int(buf[0],buf[1]) * 30 / 1000
        current_str = str( "%.2f" % current )
        data_list[index_i] = current_str.replace('.', ',')
